...
API_TOKEN = '648389668:AAGBCltsRfb0qTqo9gcq28bo0hgZ3B-mRBA'

bot = telebot.TeleBot(API_TOKEN)
user_dict = {}

allowed_users = ['191857882']
...
def send_welcome(message):
    if allow_user(message.chat.id):
        ...
        msg = bot.reply_to(message, 'Тебе сюда нельзя. Твой ID: ' + str(message.chat.id))
def choose_category(message):
        ...
        if (main_cat == u'Расход'):
                markup = types.ReplyKeyboardMarkup(one_time_keyboard=True)
                markup.add('ЖКХ','Продукты','Сотовая связь','Интернет','Проезд','Одежда','Обучение','Техника','Парикмахерская','Подарки и ДР','Медикаменты','Развлечения и отдых','Прочее')
                ...
                markup = types.ReplyKeyboardMarkup(one_time_keyboard=True)
                markup.add('Карта 4048', 'Карта 7107' , 'Карта tinkoff' ,'Наличные','Сбер счет 2601')
        ...
        try:
            with connection.cursor() as cursor:
                if user.main_cat == 'Расход':
                    ...
                    cursor.execute (sql_ins,(user.summ,user.notes,user.category,user.source))
                    ...
                    cursor.execute (sql_upd_1,(user.change_summ,user.change_source_b))
                    cursor.execute (sql_upd_2,(user.change_summ,user.change_source_a))
        ...
        try:
            with connection.cursor() as cursor:
                if user.main_cat == 'Перевод':
                    ...
                    sql_upd_1 = "UPDATE `source` SET `source_money` = `source_money` + %s WHERE `name` = %s"
                    sql_upd_2 = "UPDATE `source` SET `source_money` = `source_money` - %s WHERE `name` = %s"
                    cursor.execute (sql_ins_spent,(user.change_summ,user.change_notes_spent,user.change_category,user.change_source_a))
                    cursor.execute (sql_ins_recieve,(user.change_summ,user.change_notes_recieve,user.change_category,user.change_source_b))
                    cursor.execute (sql_upd_1,(user.change_summ,user.change_source_b))
                    cursor.execute (sql_upd_2,(user.change_summ,user.change_source_a))
                sql_sel = "SELECT `name`,`source_money` FROM `source`"
                cursor.execute (sql_sel)
            ...
            connection.commit()
        bot.send_message(chat_id, '‼️‼️‼️'+'Было переведено '+user.change_summ+ ' руб' + ' с счета ' +user.change_source_a + ' на счет ' +user.change_source_b+'‼️‼️‼️')
        bot.send_message(chat_id ,'Остаток по счетам\n' + src4048_name + ' - '+ str(int(src4048_money)) + ' руб\n' + src7107_name + ' - '+ str(int(src7107_money)) + ' руб\n' + srctinkoff_name + ' - '+ str(int(srctinkoff_money)) + ' руб\n' + srccash_name + ' - '+ str(int(srccash_money)) + ' руб\n')
    ...
    if allow_user(message.chat.id):
        ...
        try:
                with connection.cursor() as cursor:
                    ...
                    src_all = src4048_money + src7107_money + srctinkoff_money + srccash_money + srcsber1_money
        finally:
